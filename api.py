        ...
        async with async_playwright() as p:
            ...
            if STRESS_TEST_CONFIG['enabled']:
                ...
                try:
                    ...
                    # Fallback: try importing from temp_working.py
                    try:
                        from temp_working import stress_test
                        await stress_test(browser=browser, users=USERS)
                    except ImportError:
                        # If stress_test not found, run normal mode
                        logging.warning("stress_test function not found, running in normal mode")
                        # Import run_session_with_context from main.py
                        try:
                            from main import run_session_with_context
                        except ImportError:
                            from temp_working import run_session_with_context
...
# @app.get("/stress-test/status", response_model=Dict)
# async def get_stress_test_status():
#     """
#     Get the current status of the stress test.
    
#     Returns:
#         Current stress test status and information
#     """
#     global _stress_test_running, _stress_test_start_time, _stress_test_task
    
#     status_info = {
#         'running': _stress_test_running,
#         'timestamp': datetime.now().isoformat()
#     }
    
#     if _stress_test_running:
#         if _stress_test_start_time:
#             elapsed_time = time.time() - _stress_test_start_time
#             status_info['elapsed_seconds'] = round(elapsed_time, 2)
#             status_info['elapsed_minutes'] = round(elapsed_time / 60, 2)
        
#         if _stress_test_task:
#             status_info['task_done'] = _stress_test_task.done()
#             if _stress_test_task.done():
#                 try:
#                     # Check if task completed successfully
#                     await _stress_test_task
#                     status_info['completed'] = True
#                 except Exception as e:
#                     status_info['error'] = str(e)
#                     status_info['completed'] = False
#     else:
#         status_info['message'] = 'No stress test is currently running'
    
#     return status_info


# @app.post("/stress-test/configure", response_model=Dict)
# async def configure_stress_test(
#     sessions_per_user: Optional[int] = None,
#     batch_enabled: Optional[bool] = None,
#     users_per_batch: Optional[int] = None,
#     delay_between_batches: Optional[int] = None,
...
# ):
#     """
#     Configure stress test parameters including batch processing.
...
#         wait_for_completion: If True, wait for batch to complete before starting next
...
#             STRESS_TEST_CONFIG['sessions_per_user'] = sessions_per_user
#             logging.info(f"Updated sessions_per_user to {sessions_per_user}")
        
#         if batch_enabled is not None or users_per_batch is not None or delay_between_batches is not None or wait_for_completion is not None:
#             if 'batch_processing' not in STRESS_TEST_CONFIG:
#                 STRESS_TEST_CONFIG['batch_processing'] = {}
            
#             if batch_enabled is not None:
#                 STRESS_TEST_CONFIG['batch_processing']['enabled'] = batch_enabled
#                 logging.info(f"Updated batch_processing.enabled to {batch_enabled}")
            
#             if users_per_batch is not None:
#                 STRESS_TEST_CONFIG['batch_processing']['users_per_batch'] = users_per_batch
#                 logging.info(f"Updated batch_processing.users_per_batch to {users_per_batch}")
            
#             if delay_between_batches is not None:
#                 STRESS_TEST_CONFIG['batch_processing']['delay_between_batches'] = delay_between_batches
#                 logging.info(f"Updated batch_processing.delay_between_batches to {delay_between_batches}")
            
#             if wait_for_completion is not None:
#                 STRESS_TEST_CONFIG['batch_processing']['wait_for_completion'] = wait_for_completion
#                 logging.info(f"Updated batch_processing.wait_for_completion to {wait_for_completion}")
        
#         return {
#             'status': 'success',
#             'message': 'Configuration updated',
#             'config': {
#                 'sessions_per_user': STRESS_TEST_CONFIG.get('sessions_per_user'),
#                 'batch_processing': STRESS_TEST_CONFIG.get('batch_processing', {})
#             },
#             'timestamp': datetime.now().isoformat()
#         }
...
# @app.post("/stress-test/stop", response_model=Dict)
# async def stop_stress_test():
#     """
#     Stop the currently running stress test.
    
#     Note: This will attempt to cancel the stress test task, but browser cleanup
#     may take some time.
...
#     try:
#         if _stress_test_task and not _stress_test_task.done():
#             _stress_test_task.cancel()
#             try:
#                 await _stress_test_task
...
#         }
#     except Exception as e:
#         raise HTTPException(status_code=500, detail=f"Error stopping stress test: {str(e)}")
